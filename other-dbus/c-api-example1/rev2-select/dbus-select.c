...
static dbus_bool_t add_timeout(DBusTimeout *t, void *data)
{
    ...
    printf(LOG_DEBUG " adding timeout %p\n", t);
    watched_timeout = t;
...
int dbus_selector(char *param)
{
   ...
   // request our name on the bus and check for errors
   ret = dbus_bus_request_name(conn, "test.actor.server", 
        ...
        if ( watched_watch != NULL ) {
            if ( watched_rd_fd ) { 
                if ( FD_ISSET(watched_rd_fd, &rfds) ) {
                    ...
                    dbus_selector_process_recv(conn, 0);
                }
    ...
    do {
        ...
        if ( mtype == DBUS_MESSAGE_TYPE_METHOD_RETURN ) {
            ...
            if ( iswaiting_rpcreply ) {
                ...
                printf(" signal pending check SUCCESS: received signal. \n");
